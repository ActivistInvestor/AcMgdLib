...
namespace Autodesk.AutoCAD.ApplicationServices.Extensions
{
   ...
   {
      /// <summary>
      /// Commits the transaction if it has 
      /// not already been ended.
      /// </summary>
      /// <returns></returns>

      public static bool TryCommit(this Transaction trans)
      {
         Assert.IsNotNullOrDisposed(trans, nameof(trans));
         bool result = trans.AutoDelete;
         if(result)
            trans.Commit();
         return result;
      }

      public static bool TryAbort(this Transaction trans)
      {
         Assert.IsNotNullOrDisposed(trans, nameof(trans));
         bool result = trans.AutoDelete;
         if(result)
