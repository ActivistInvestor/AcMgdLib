...
namespace Autodesk.AutoCAD.Runtime.LispInterop
{
   ...
   public static class ListBuilder
   {
      ...
      ///   
      ///   The result can be returned by a LispFunction having
      ///   a return type of ResultBuffer.
      ///   
      /// </summary>
      ...
      public static ResultBuffer ToAssociationList<T>(this IEnumerable<T> src,
         LispDataType carType,
         LispDataType cdrType,
         Func<T, object> car,
         Func<T, object> cdr)
      {
         if(src == null)
         ...
         if(!src.Any())
            return null;
         short typeCar = (short)carType;
         short typeCdr = (short)cdrType;
         var begin = new TypedValue((short)LispDataType.ListBegin);
         var end = new TypedValue((short)LispDataType.DottedPair);
         var rb = new ResultBuffer();
         foreach(T item in src)
         {
            rb.Add(begin);
            rb.Add(new TypedValue(typeCar, car(item)));
            rb.Add(new TypedValue(typeCdr, cdr(item)));
            rb.Add(end);
         }
         return rb;
      }
      ...
      /// to this method and the result passed to List(), elements
      /// of the argument are promoted to become elements of the 
      /// containing list.
      ///
      ...
      static readonly Dictionary<LispDataType, Type> lispDataTypeToTypeMap = 
         new Dictionary<LispDataType, Type>()
      {
