...
namespace AcMgdLib.Common.Examples
{
   ...
   { 
      ...
      {
         var res = typeof(LispInteropTests).GetMethods(BindingFlags.Public | BindingFlags.Static)
            .Where(mi => mi.IsDefined(typeof(LispFunctionAttribute), false))
            .Select(mi => mi.GetCustomAttribute<LispFunctionAttribute>());

         foreach(var lfa in res)
         {
            lfa.GlobalName.WriteLine();
         }
      }
   }
