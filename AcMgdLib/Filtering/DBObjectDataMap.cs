...
namespace Autodesk.AutoCAD.DatabaseServices.Extensions
{
   ...
   /// obtain an ToObjectId that is used as the key to store the
   ...
   /// which is an ToObjectId. That ToObjectId is used as the key to store
   /// the cached data. The cache takes the ToObjectId and looks for data
   /// in the cache using that ToObjectId as a key. 
   /// 
   /// If data for the ToObjectId is found in the cache, it is returned. 
   /// Otherwise, the DBObjectDataMap opens the DBObject referenced by 
   /// the ToObjectId and passes the opened DBObject to the second delegate 
   /// (which takes a LayerTableRecord as its argument) which returns the 
   /// data that is to be cached, keyed to the ToObjectId returned by the 
   /// first delegate. It then adds the resulting data to the cache and 
   /// returns it. 
   /// 
   /// On subsequent requests for data for that same ToObjectId/key, it is 
   ...
   /// 
   /// 1. An ToObjectId that references a TCriteria must be 
   ///    reachable using an instance of a TFiltered.
   ///    
   /// 2. A TValue must be reachable using an instance of a 
   ///    TCriteria.
   /// 
   /// 3. The ToObjectId obtained from a TFiltered is a key that is
   ///    permanently associated with the TValue obtained from the
   ///    TCriteria referenced by the ToObjectId key.
   /// 
   /// </summary>
      ...
      where TValueSource : DBObject
   {
      ...
      /// <summary>
      /// If the keySelector returns ToObjectId.Null, this
      /// will be called to provide a value (or throw an
      /// exception, if appropriate).
      /// 
      /// In the EffectiveColorMap, the keySelector returns
      /// ToObjectId.Null to signal that the entity argument's
      ...
      /// 
      /// </summary>
      /// <param name="id">The ToObjectId of a TCriteria object</param>
      ...
      /// on an instance of a TCriteria having the given ToObjectId</returns>
      ...
      /// specified key.
      /// </summary>
      /// <param name="id">The ToObjectId key of a <typeparamref name="TValueSource"/>
