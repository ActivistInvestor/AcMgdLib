...
namespace Autodesk.AutoCAD.DatabaseServices.Extensions
{
   ...
   public abstract partial class BlockVisitor<T> where T: BlockVisitor<T>.StackEntry
   {
      ...
      HashSet<Type> unhandledTypes = new HashSet<Type>();
      bool skipUnhandledTypes = true;
      Stack<BlockReference> containers = new Stack<BlockReference>();
      protected readonly ReferenceStack stack;
      readonly List<string> path = new List<string>();
      Transaction trans;
      ...
      readonly Cached<Matrix3d> blockTransform = null;
      bool userBlocksOnly = false;
      bool visitAttributes = false;
      readonly List<ObjectId> subentPath = new List<ObjectId>() { ObjectId.Null };
      protected readonly Func<ObjectId, bool> predicate;
      ...
      /// <summary>
      /// Indicates if the default handler was
      /// invoked on the Type of the entity that
      /// was just visited.
      /// </summary>
      private bool defaultHandlerInvoked;

      /// <summary>
      /// Indicates if at least one entity in
      /// the definition of the block reference 
      /// currently being visited was modified.
      /// This variable has block reference scope.
      /// </summary>
      private bool blockModified;

      /// <summary>
      /// Indicates if at least one entity in
      /// the definition of any block that was
      /// visited was modified. This variable
      /// has operation scope.
      /// </summary>
      private bool objectsModified;
      ...
      public void Visit(ObjectId id, Transaction trans, bool nested = false)
      {
         ...
         finally
         {
            ...
            FinalizeInternal(id, failed);
         }
      }

      private void FinalizeInternal(ObjectId id, bool failed)
      {
         visiting = false;
         Finalize(id, failed);
      }
      ...
      /// when visiting begins:
      
      void InitializeInternal(ObjectId id)
      {
         this.root = null;
      ...
      /// which references the BlockTableRecord that directly owns the
      /// entities currently being visited.
      /// 
      ...
      protected virtual void VisitBlockReference(BlockReference blkref)
      {
         ...
         if(blockModified)
            OnModified(blkref);
         /// Overrides of this can also modify block entities,
         /// which cannot be detected without iterating over
         /// the entities in the block definition, or by using
         /// an ObjectOverrule.
         OnVisited(blkref);
         Pop();
         if(VisitAttributes)
         {
            ...
            if(attributes.Count > 0)
            {
               ...
               try
               {
                  ...
                  {
                     var attref = trans.GetObject<AttributeReference>(id);
                     if(!attref.IsConstant)
                        DynamicDispatch(attref);
                  }
               }
               finally
               {
      ...
      protected virtual void OnModified(BlockReference blkref)
      {
      }

      /// <summary>
      /// Indicates if the instance should call 
      /// RecordGraphicsModified() on the root block 
      /// reference when one or more nested entities 
      /// is modified.
      /// </summary>
      public bool RecordGraphicsModified { get;set; }
      ...
      {
         if(skipUnhandledTypes && unhandledTypes.Contains(entity.GetType()))
            return;
         currentEntity = entity;
         bool flag = entity.IsModified;
         defaultHandlerInvoked = false;
         dynamicThis.Visit((dynamic)entity);
         if(flag ^ entity.IsModified)
         {
            objectsModified = true;
            blockModified = true;
         }
         if(skipUnhandledTypes && defaultHandlerInvoked)
            unhandledTypes.Add(entity.GetType());
         ++visitedCount;
      }

      /// <summary>
      /// Indicates if at least one entity in the definition
      /// of the block reference currently being visited was 
      /// modified. This property has block scope, and is set
      /// to false prior to visiting each block reference.
      /// </summary>
      protected bool BlockModified => blockModified;
      ...
      /// <summary>
      /// Invoked immediately after a block is
      /// visited. if one or more entities in 
      /// the block's definition was modified.
      /// </summary>
      /// <param name="entity"></param>

      private void OnModified(Entity entity)
      {
         // Need a stack element scoped variable 
         // In the interim, this variable must be
         // reset on each recursion into a block
         // referenced, and checked on return
         // and acted on accordingly. This is one
         // of the problems that would be solved by
         // refactoring to use a custom stack frame
