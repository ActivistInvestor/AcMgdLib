...
namespace Autodesk.AutoCAD.DatabaseServices.Extensions
{
   ...
   /// 
   /// The GetContext() method must be overridden to create an 
   /// instance for each Block Reference that is visited. 
   /// This class stores the ObjectIds of the entities that 
   /// it clones in the ObjectIdCollection.
   /// 
   ...
   /// nested in, a seperate deep-clone operation must be
   /// performed with only those objects that are directly-
   /// nested within a single block reference's definition.
   /// 
   /// Special Notes:
   /// 
      ...
      where T : Entity 
   {
      ...
      ObjectIdCollection results = new ObjectIdCollection();
      ...
      {
         if(HasContext && !BlockId.IsNull)
         {
            var map = Current.CopyTo(BlockId, Transform);
            foreach(ObjectId id in map.GetPrimaryCloneIds())
               results.Add(id);
         }
      ...
      public ObjectIdCollection Result => results;
   }
