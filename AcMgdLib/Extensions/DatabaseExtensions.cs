...
namespace Autodesk.AutoCAD.DatabaseServices.Extensions
{
   ...
   public static partial class DatabaseExtensions
   {
      ...
      /// <summary>
      /// Opens and returns the model space BlockTableRecord for the given Database.
      /// </summary>

      public static BlockTableRecord GetModelSpaceBlock(this Database db,
         Transaction trans,
         OpenMode mode = OpenMode.ForRead)
      {
         return trans.GetObject<BlockTableRecord>(db.GetModelSpaceBlockId(), mode);
      }

      /// <summary>
      /// Opens and returns the current space BlockTableRecord for the given Database.
      /// </summary>

      public static BlockTableRecord GetCurrentSpaceBlock(this Database db,
         Transaction trans,
         OpenMode mode = OpenMode.ForRead)
      {
         return trans.GetObject<BlockTableRecord>(db.CurrentSpaceId, mode);
      }
         ...
         bool throwIfNotFound = true) where T : DBObject
      {
         ...
         return db.GetDictionaryObject<T>(key, trans, mode, throwIfNotFound);
      }
         ...
         bool openErased = false) where T : DBObject
      {
         if(typeof(SymbolTableRecord).IsAssignableFrom(typeof(T)))
         {
            ...
            var table = func(db).GetObject<SymbolTable>(trans);
            if(openErased)
               table = table.IncludingErased;
            return table.GetObjectsOfType<T>(trans, true, mode, openErased);
         }
      ...
      /// <summary>
      /// Opens an object from the Named Objects Dictionary
      /// </summary>
      ...
      public static T GetNODObject<T>(this Database db, 
         Transaction trans, 
         string key, 
         OpenMode mode = OpenMode.ForRead,
         OnKeyNotFound action = OnKeyNotFound.Throw) where T: DBObject, new()
      {
         db.CheckTransaction(trans);
         var nod = trans.GetObject<DBDictionary>(db.NamedObjectsDictionaryId);
         if(nod.Contains(key))
            return trans.GetObjectChecked<T>(nod.GetAt(key), mode);
         if(action == OnKeyNotFound.Create)
         {
            T result = new T();
            nod.SetAt(key, result);
            trans.AddNewlyCreatedDBObject(result, true);
            return result;
         }
         else if(action == OnKeyNotFound.Throw)
            throw new KeyNotFoundException(key);
         return default(T);
      }
      ...
      {
         db.CheckTransaction(trans);
         var dict = DBDictionary<T>.GetDictionary(db, trans);
         if(openErased)
            dict = dict.IncludingErased;
         return dict.GetObjects<T>(trans, mode, openErased);
      }
      ...
      public static void SetUndoRequiresRegen(this Database database)
      {
         Assert.IsNotNullOrDisposed(database, nameof(database));
         CoreUtils.SetUndoRequiresRegen(database);
      }
         ...
         Func<BlockTableRecord, bool> predicate = null)
      {
         ...
         Assert.IsNotNullOrWhiteSpace(pattern, nameof(pattern));
         predicate = predicate ?? BlockTableRecordExtensions.IsUserBlock;
      ...
      /// </summary>
      /// <param name="blkref">The BlockReference to retrieve the
      /// attribute tags and values from</param>
      /// <param name="tr">The Transaction to use in the operation</param>
      /// <param name="mode">The OpenMode to use to open the 
      /// AttributeReferences with</param>
      /// <returns>A Dictionary containing the attribute tags
      /// as the keys and</returns>
      ...
      /// <summary>
      /// Returns a Dictionary containing the Tags and
      /// Values of some or all AttributeReferences owned 
      /// by the given BlockReference.
      /// </summary>
      /// <param name="blkref">The BlockReference to retrieve 
      /// the attribute tags and values from</param>
      /// <param name="tr">The Transaction to use in the 
      /// operation</param>
      /// <param name="tags">One or more strings indentifying
      /// the tags of AttributeReferences whose tag and value
      /// is to be included in the result. If no values are 
      /// provided, all tags and values are returned in the 
      /// resulting dictionary.</param>
      /// <returns>A Dictionary containing one or more pairs
      /// of tags and values, or an empty dictionary if tag
      /// arguments were provided and none of the specified
      /// tags were found.</returns>
         ...
         params string[] tags)
      {
         ...
         {
            if(tagset.Contains(att.Tag))
            {
               result[att.Tag] = att.TextString;
               if(result.Count == tagset.Count)
                  break;
            }
         }
         return result;
      ...
      //public static IEnumerable<T> GetAttributeInfo<T>(this Database db,
      //   string pattern,
      //   Func<BlockReference, Dictionary<string, string>, T> selector,
      //   params string[] tags)
      //{
      //   Assert.IsNotNullOrDisposed(db, nameof(db));
      //   if(string.IsNullOrWhiteSpace(pattern))
      //      pattern = "*";
      //   using(var tr = new DatabaseTransaction(db))
      //   {
      //      var items = db.GetBlockReferences(pattern, tr, OpenMode.ForRead,
      //            btr => btr.IsUserBlock() && btr.HasAttributes(tr, tags))
      //         .Select(br => new AttributeInfo(br, tr, tags))
      //         .ToArray();
      //      tr.Commit();
      //      return items;
      //   }
      ...
      class WorkingDatabase : DisposableBase
      {
         Database prevWorkingDatabase;

         public WorkingDatabase(Database db)
         {
            Assert.IsNotNullOrDisposed(db, nameof(db));
            Database temp = HostApplicationServices.WorkingDatabase;
            if(temp != db)
            {
               prevWorkingDatabase = temp;
         ...
         {
            if(disposing && prevWorkingDatabase != HostApplicationServices.WorkingDatabase)
            {
               if(prevWorkingDatabase != null && !prevWorkingDatabase.IsDisposed)
               {
                  HostApplicationServices.WorkingDatabase = prevWorkingDatabase;
                  prevWorkingDatabase = null;
               }
            }
         }
      }
   ...
   public enum OnKeyNotFound
   {
      Default = 0,  // Return default value if key not found
      Throw = 1,    // Throw an exception if key not found
      Create = 2,   // Create new entry if key not found
      Add = Create
   }
}
