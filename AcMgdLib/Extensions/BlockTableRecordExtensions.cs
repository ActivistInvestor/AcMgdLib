   ...
   public static partial class BlockTableRecordExtensions
   {
      public static ObjectId Copy(this BlockTableRecord btr, string newName, bool cloneDrawOrder = true, Action<BlockTableRecord, BlockTableRecord, IdMapping> OnCloned = null)
      {
         ...
         using(Database dbTemp = new Database(true, true))
         {
            ...
            dbTemp.WblockCloneObjects(ids, db.BlockTableId, map2, DuplicateRecordCloning.Ignore, false);
      ...
      public static Func<BlockTableRecord, bool> IsUserBlockPredicate(this IEnumerable<BlockTableRecord> blocks)
      {
         return IsUserBlock;
      }
      ...
      /// contains all of the non-constant attribute definitions 
      /// whose tags are passed as arguments.
      /// 
      /// This API requires that all attribute definitions
      /// have distinct tag strings, and that all tags that
      /// are passed as arguments be distinct.
      /// </summary>
      /// <param name="btr">The BlockTableRecord to operate on</param>
      /// <param name="tr">The transaction to use in the operation</param>
      /// <param name="tags">One or more strings representing the
      /// tags of required attributes.</param>
      /// <returns></returns>

      public static bool HasAttributes(this BlockTableRecord btr, Transaction tr, params string[] tags)
      {
         Assert.IsNotNullOrDisposed(btr, nameof(btr));
         ...
         int cnt = tags.Length;
         HashSet<string> targets = new HashSet<string>(tags, StringComparer.OrdinalIgnoreCase);
         if(cnt != targets.Count)
            throw new ArgumentException("tag arguments must be distinct");
         int found = 0;
         foreach(var attdef in btr.GetObjects<AttributeDefinition>(tr))
         {
            if(!attdef.Constant && targets.Contains(attdef.Tag))
            {
               if(++found == targets.Count)
                  return true;
            }
         }
         return false;
      ...
      /// <summary>
      /// Enumerates some or all AttributeDefinitions from the
      /// given BlockTableRecord.
      /// </summary>
      /// <param name="btr">The BlockTableRecord to operate on</param>
      /// <param name="trans">The Transaction to use in the operation</param>
      /// <param name="mode">The mode to open AttributeReferences as</param>
      /// <param name="tags">Zero or more strings that identify the tags of
      /// the requested AttributeReferences. If no values are provided, all
      /// AttributeReferences are enumerated.</param>
      /// <returns>An object that enumerates the requested AttributeDefinitions</returns>
      /// <exception cref="ArgumentException"></exception>
      
      public static IEnumerable<AttributeDefinition> GetAttributeDefs(this BlockTableRecord btr,
         Transaction trans,
         OpenMode mode = OpenMode.ForRead,
         params string[] tags)
      {
         ...
         if(tags != null && tags.Length > 0)
         {
            if(tags.Any(string.IsNullOrEmpty))
               throw new ArgumentException("invalid tag");
            targets = new HashSet<string>(tags, StringComparer.OrdinalIgnoreCase);
            if(targets.Count != tags.Length)
               throw new ArgumentException("tag arguments must be distinct");
         }
         int yielded = 0;
         foreach(var attdef in btr.GetObjects<AttributeDefinition>(trans))
         {
            if(!attdef.Constant && targets == null || targets.Contains(attdef.Tag))
            {
               ++yielded;
               yield return attdef;
               if(targets != null && yielded == targets.Count)
                  yield break;
            }
         }
      }
