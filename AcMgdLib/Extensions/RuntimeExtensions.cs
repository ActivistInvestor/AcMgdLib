...
namespace Autodesk.AutoCAD.Runtime
{
   ...
   public static partial class RXClass<T> where T : RXObject
   {
      ...
      {
         if(includingErased)
            return exactMatch && !typeof(T).IsAbstract ? (Func<ObjectId, bool>) IsMatchExact : IsMatch;
         else
            return exactMatch && !typeof(T).IsAbstract ? (Func<ObjectId, bool>) NonErasedIsMatchExact : NonErasedIsMatch;
      }
