   ...
   { 
      ...
      public static void TestNewObjectCollection()
      {
         ...
         if(newLineObserver == null)
         {
            ...
            /// If this property is not set to true, there will
            ...
            newLineObserver.NotifyOnQuiscence = true;
         }
         else
         {
            what = "disabled";
      ...
      {
         AcConsole.Write($"\nCollectionChanged: AddedCount: {e.AddedCount}  Count: {e.Count}  NonErasedCount: {e.NonErasedCount}");
         using(var tr = new DocumentTransaction())
         {
            foreach(Line line in e.NewObjectIds.GetObjects<Line>(tr))
            {
               AcConsole.Write($"Line {line.ObjectId.ToString()} IsErased = {line.IsErased}");
            }
            tr.Commit();
         }
      }
